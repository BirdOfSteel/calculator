...
// this function loops over characterListArray to make an array of keypad buttons
export default function RenderKeypadButtons({addToEquationArray, styleState}) {
    const buttonList = characterListArray.map((charObject, index) => {
        ...
        return (
            <button key={index} onClick={(event) => { addToEquationArray(event) } } className="keypad-btn" id={`key-${keyID}`} style={{backgroundColor: styleState.inputBackground, boxShadow: styleState.inputBoxShadow, color: styleState.textColour}}>{keyText}</button>
        )
    })
